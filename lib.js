// Shared lib for both simulators. Loads config.yaml and exposes formulas + small UI helpers.
...
async function loadConfig() {
  ...
  if (!r.ok) throw new Error('config.yaml fetch failed — serve via http (python3 -m http.server)');
